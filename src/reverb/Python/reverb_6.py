...
def main():
    ...
    plt.title("Response of feedback filters (linearly spaced coeffs from 3500 to 4300)")
    plt.xlabel("Frequency")
    plt.ylabel("Amplitude (dB)")
    plt.show()

    plt.plot(np.arange(0, result.shape[0]//2), result[0:result.shape[0]//2])
    plt.title("Impulse response of reverb (linearly spaced coeffs from 3500 to 4300")
    plt.show()

    # write_audio_safe("Processed flintstones with random coefficients (reverb 6).wav", samp_rate, result)

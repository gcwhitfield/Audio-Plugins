...
void ReverbAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    prev_audio_buffer.clear();
    prev_audio_buffer.setSize(2, samplesPerBlock);
    for (int channel = 0; channel < prev_audio_buffer.getNumChannels(); channel++) {
        auto *data = prev_audio_buffer.getWritePointer(channel);
        for (size_t i = 0; i < prev_audio_buffer.getNumChannels(); i++) {
            data[i] = 0;
        }
    }
}
...
void ReverbAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        float g = 0.7; // -3db of attenuation for every loop around the circuit
        int delay_amount = 500; // the amount of delay, in samples
        for (int i = 0; i < buffer.getNumSamples(); i++) {
            float delayed_sample = 0;
            if (i - delay_amount < 0) {
                delayed_sample = prev_audio_buffer.getReadPointer(channel)[prev_audio_buffer.getNumSamples() + i - delay_amount];
            } else {
                delayed_sample = channelData[i - delay_amount];
            }
            channelData[i] = delayed_sample + g * prev_sample;
            prev_sample = channelData[i];
        }
    }
    
    prev_audio_buffer.makeCopyOf(buffer);
}
